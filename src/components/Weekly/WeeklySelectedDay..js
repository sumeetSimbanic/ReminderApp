...
const WeeklySelectedDay = () => {
  ...
  const calculateNextOccurrences = (selectedDays, selectedTime, startDate) => {
    const now = new Date();
    const selectedDayIndices = selectedDays.map(day => ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].indexOf(day));
    
    const nextOccurrences = selectedDayIndices.map(dayIndex => {
    ...
    // Filter out occurrences that are in the past
    const filteredOccurrences = nextOccurrences.filter(occurrence => occurrence > now);
    
    // Convert the remaining occurrences to date strings
    const nextOccurrenceStrings = filteredOccurrences.map(occurrence => occurrence.toDateString());
  
    return nextOccurrenceStrings;
  };
  ...
  renderItem={({ item, index }) => (
    <View style={{ marginVertical: 10 }}>
      ...
      <Text>Next Occurrences: {calculateNextOccurrences(item.selectedDays, item.selectedTime, item.startDate).join(', ')}</Text>
...
export default WeeklySelectedDay;
